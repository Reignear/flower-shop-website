...
import { useAdminOrderDelivered } from "@/hooks/use-admin-order-delivered";

export default function OrderDelivered() {
  const { data: orders, isLoading: isOrdersLoading } =
    useOrderByStatus("delivered");
  const { activeLayout, setActiveLayout } = useAdminOrderDelivered();
  return (
    <AdminLayout className="p-8" breadCrumbs={orderDeliveredBreadCrumb}>
                ...
                >
                  <div className="p-6 space-y-4">
                    ...
                    <div className="flex gap-2 pt-2">
                      <Link
                        to={`/admin/order/delivered/${order.id}`}
                        className="w-full"
                      >
