...
import { useOrderPending } from "@/hooks/use-user-order-pending";
import {
  Calendar,
  CalendarCheck2,
  ChevronRight,
...
import { usePendingBreadCrumb } from "@/data/user-order-data";
const OrderPending = () => {
  const { data: Orders, isLoading: isOrdersLoading } =
    useOrderByStatus("pending");
  const { imgLoaded, setImgLoaded } = useOrderPending();
  return (
    <Order breadCrumbs={usePendingBreadCrumb}>
      {Orders?.map((order: OrderType, index: number) => (
        ...
        >
          ...
          <div className="px-4 md:px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 ">
            <Link to={`/user/order/pending/${order.id}`} className="w-full">
