...
import { useUserOrderForPickup } from "@/hooks/use-user-order-for-pickup";
...
import { Calendar, CalendarCheck2, ChevronRight, MapPin, Package } from "lucide-react";
import { Mosaic } from "react-loading-indicators";
import { Link } from "react-router-dom";

const OrderForPickup = () => {
  const { data: Orders, isLoading: isOrdersLoading } =
    useOrderByStatus("for-pickup");
  const { imgLoaded, setImgLoaded } = useUserOrderForPickup();
  return (
    <Order breadCrumbs={useForPickupBreadCrumb}>
      {Orders?.map((order: OrderType, index: number) => (
        ...
        >
          ...
          <div className="px-4 md:px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 ">
            <Link to={`/user/order/for-pickup/${order.id}`} className="w-full">
