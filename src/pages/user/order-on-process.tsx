...
import { useUserOrderPending } from "@/hooks/use-user-order-on-process";
import { useOnProcessBreadCrumb } from "@/data/user-order-data";

export default function OrderOnProcess() {
  const { data: Orders, isLoading: isOrdersLoading } =
    useOrderByStatus("on-process");
  const { imgLoaded, setImgLoaded } = useUserOrderPending();
  return (
    <Order breadCrumbs={useOnProcessBreadCrumb}>
      {Orders?.map((order: OrderType, index: number) => (
        ...
        >
          ...
          <div className="px-4 md:px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 ">
            <Link to={`/user/order/on-process/${order.id}`} className="w-full">
      ...
      {Orders && Orders.length === 0 && (
        <div className="text-center py-10">
          <p className="text-gray-500 text-sm">No On Process orders found.</p>
        </div>
      )}
    </Order>
  );
